...
class TestFriendsPage(BaseTest):
    ...
    def test_send_message_to_friend(self, add_users):
        ...
        self.friends_page().enter_message_text(message_text=self.post_text)
        self.friends_page().click_send_button()
        self.friends_page().is_message_sent(message_text=self.post_text)
        ...
        self.news_feed_page(admin).top_bar_menu.messages.is_inbox_message_received_from(friend_name="Artem Lukevich")